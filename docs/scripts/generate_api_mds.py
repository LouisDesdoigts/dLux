...
def render_section_overview(section: str, entries: list[tuple[str, str]]) -> str:
    """Render a compact linked map of the modules in one API section."""
    out = [
        GENERATED_NOTICE,
        "",
        f"# {section_title(section)}",
        "",
        "This map is generated from the public API. Select a module to open its "
        "classes, functions, and local inheritance diagram.",
        "",
        "```mermaid",
        "flowchart LR",
    ]
    ...
    for stem, module in entries:
        ...
        ident = re.sub(r"\W", "_", module)
        summary = f"Public API: {', '.join(names)}" if names else "No public exports"
        out.append(f'    {ident}["{display_name(stem)}"]')
        out.append(f'    click {ident} href "../{stem}/" "{summary}"')
...
def render_package_overview(modules: dict[str, list[tuple[str, str]]]) -> str:
    """Render the package-level conceptual map at documentation width."""
    counts = {section: len(modules[section]) for section in SECTIONS}
    labels = {
    ...
    for section in SECTIONS:
        tooltip = f"{counts[section]} public modules"
        lines.append(f'    {section}["{labels[section]}"]')
        lines.append(f'    click {section} href "{section}/" "{tooltip}"')
    lines.extend(
        [
            "    utils --> core",
            "    utils --> parametric",
    ...
    return "\n".join(
        [
            ...
            "# API",
            "",
            "This generated map separates the main dLux contracts at a readable "
            "scale. Select a section for its module map, then a module for local "
            "inheritance and complete API documentation.",
            "",
            *lines,
            "```",
            "",
