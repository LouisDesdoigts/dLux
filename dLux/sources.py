...
Source classes store `Spectrum` objects that define the spectral properties of
the source. They also store the position of the source on the sky, and the flux
of the source. 

All public clases have two main methods:

1. `.normalise()`
> Returns a new source object with the relevant attributes normalised.
...
Lets have a look at how to construct all of these different classes. First we
construct some optics to model the sources through.
