...
===================================
This module contains the classes that define the behaviour of DetectorLayers in
dLux.
...
These classes operate on `Image` classes. They have one main method,
`.__call__(image)` that takes in a dLux `Image` class and applies the detector
layer to it.
...
## ApplyJitter
Convolves the image wi

APIth a gaussian kernel parameterised by the standard
deviation (sigma).
