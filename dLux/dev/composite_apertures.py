...
class CompoundAperture(CompositeAperture):
    ...
    def _coordinates(self: ApertureLayer, coords: Array) -> Array:
        ...
        coords: float = coords - self.centre
        coords: float = coords + np.transpose(coords, (0, 2, 1)) * self.strain
        coords: float = coords * self.compression
    ...
    def _aperture(self, coords: Array) -> Array:
        ...
        aperture : Matrix
           ...
           apertures that were stored. 
        """
        coords: float = self._coordinates(coordinates)
