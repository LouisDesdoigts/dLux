...
class BroadbandWavefront(Wavefront):
    def __init__(
        self: Wavefront, npixels: int, diameter: Array, wavelengths: Array
    ):
        """
        Constructor for the wavefront.
        ...
        """
        super().__init__()
        self.wavelength = np.array(wavelengths, dtype=float)
        self.pixel_scale = np.array(diameter / npixels, dtype=float)

        # Ensure dimensionality
        if self.wavelength.ndim != 1:
            raise ValueError("Wavelength must be a 1D array of wavelengths.")

        # Initialise amplitude and phase
        nwavels = len(self.wavelength)
        self.amplitude = (
            np.ones((nwavels, npixels, npixels), dtype=float) / npixels**2
        )
        self.phase = np.zeros((nwavels, npixels, npixels), dtype=float)
    ...
    def psf(self: Wavefront) -> Array:
        ...
        psf : Array
            The PSF of the wavefront.
        """
        return np.sum(self.amplitude**2, 0)
    ...
    def coordinates(self: Wavefront) -> Array:
        ...
        coordinates : Array
            ...
            wavefront.
        """
        # Check pixel scale dims in case we have FFTd to a focal plane
        if self.pixel_scale.shape == ():
            coord_fn = dlu.pixel_coords
        else:
            coord_fn = vmap(dlu.pixel_coords, (None, 0))
        return coord_fn(self.npixels, self.pixel_scale)

    @staticmethod
    def _multiply(vector, array):
        """
        Automatically handles the potential dimensionality miss-match between
        the input vector (always one dimensional) and the array, which can have
        either one, two or three dimensions.
        """
        if array.ndim == 3:
            return vector[:, None, None] * array
        elif array.ndim == 2:
            return vector[:, None, None] * array[None, ...]
        else:
            return vector * array

    # TODO: Is the actually needed?
    @staticmethod
    def _add(vector, array):
        """
        Automatically handles the potential dimensionality miss-match between
        the input vector (always one dimensional) and the array, which can have
        either one, two or three dimensions.
    ...
    def add_opd(self: Wavefront, path_difference: Array) -> Wavefront:
        ...
        wavefront : Wavefront
            ...
            path_difference
        """
        phase_difference = self._multiply(self.wavenumber, path_difference)
        return self.add("phase", phase_difference)
    ...
    def tilt(self: Wavefront, angles: Array) -> Wavefront:
        ...
        # Dimensionality check as coords can be 3 or 4 dimensional
        coords = self.coordinates
        if coords.ndim == 3:
            opd = -(angles[:, None, None] * coords).sum(0)
        else:
            opd = -(angles[None, :, None, None] * coords).sum(1)
        return self.add_opd(opd)
    ...
    def flip(self: Wavefront, axis: tuple) -> Wavefront:
        ...
        wavefront : Wavefront
            The new wavefront with the flipped amplitude and phase.
        """
        # Increase the axis by one to account for the extra dimensionality
        return super().flip(axis + 1)

    # #################################################################### #
    # ALL the below functions need to be updated to work within this class #
    # #################################################################### #
