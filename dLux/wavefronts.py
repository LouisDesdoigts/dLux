...
===================================
This module contains the classes that define the behaviour of Wavefronts in
dLux.
...
The wavelength attribute is the wavelength of the wavefront. The amplitude and
phase attributes are the amplitude and phase of the wavefront. The pixel_scale
is the physical size of the pixels representing the wavefront. The plane
attribute is the current plane of the wavefront, which can be 'Pupil', 'Focal'
or 'Intermediate'. The units attribute is the current units of the wavefront,
which can be 'Cartesian' or 'Angular'.

It in important to note here that the `plane` and `units` attributes do not
actually determine the behaviour of the wavefront, but rather are used to
track the current unit type. Ie if you set the units parameter from 'Cartesian'
to 'Angular' the wavefront will not be converted to angular units, but rather
the units will be _treated_ as angular in propagations, which can lead to
incorrect results.
...
The wavefront classes contain a series of methods that allow for the
manipulation and propagation of the wavefron. It also has a series of operators
that allow for the manipulation of the wavefront via standard arithmetic
operators, ie:
...
## fringe_size
Returns the size of the fringes in angular units, ie lambda/D.
...
## add_opd
Applies the input array as an OPD to the Wavefront.
API
...
the pixel_scale_out and npixels parameters.
API

## rotate
Performs a paraxial rotation on the wavefront, determined by the
the angle parameter, using interpolation.
API
...
## crop_to
Paraxially crops the `Wavefront` to the size determined by
npixels. Note this only supports cropping arrays of even dimension
to even dimension, and odd dimension to to odd dimension, ie 4 -> 2 or
5 -> 3.
API
...
## FFT(pad, focal_length=None)
Performs a Fast Fourier Transform (FFT) on the wavefront, propagating from
Pupil to Focal planes. If the focal_length is None, the output units will
angular, otherwise cartesian.
API

## IFFT(pad, focal_length=None)
Performs an Inverse Fast Fourier Transform (IFFT) on the wavefront, propagating
from Focal to Pupil planes.
API

## MFT(npixels, pixel_scale, focal_length=None)
Performs a Matrix Fourier Transform (MFT) on the wavefront, propagating from
Pupil to Focal planes. If the focal_length is None, the pixel_scale is assumed
to be in angular units (radians), otherwise it is assumed to be in cartesian
units (meters).
API

## IMFT(npixels, pixel_scale, focal_length=None)
Performs an Inverse Matrix Fourier Transform (IMFT) on the wavefront,
propagating from Focal to Pupil planes.
API

## shifted_MFT(npixels, pixel_scale, shift, focal_length=None, pixel=True)
Performs a Matrix Fourier Transform (MFT) on the wavefront, propagating from
Pupil to Focal planes. If the focal_length is None, the pixel_scale is assumed
to be in angular units (radians), otherwise it is assumed to be in cartesian
units (meters). The shift parameter is used to shift the center of the output
plane by 'shift', which is treated in units of pixels by default, otherwise
it is treated in the units of the pixel_scale.
API

## shifted_IMFT(npixels, pixel_scale, shift, focal_length=None, pixel=True)
Performs an Inverse Matrix Fourier Transform (IMFT) on the wavefront,
propagating from Focal to Pupil planes. The shift parameter is used to shift
the center of the output plane by 'shift', which is treated in units of pixels
by default, otherwise it is treated in the units of the pixel_scale.
API
...
The FresnelWavefront class is a subclass of the Wavefront, implementing the
methods required to perform a FarFieldFresnel propagation (ie, close to the
focal plane). It implements two more methods that are essential extensions of
the MFT methods. This class _only_ implements cartesian propagation and only
propagates from 'Pupil' to 'Focal' planes.

## fresnel_prop(npixels, pixel_scale, focal_length, focal_shift)
Performs a Fresnel propagation on the wavefront, propagating from
Pupil to Focal planes. The focal_shift parameter represents the distance
from the focal plane at which the PSF is modelled.
API

## shifted_fresnel_prop(npixels pixel_scale, shift, focal_length, focal_shift,
    pixel=True)
Performs a Fresnel propagation on the wavefront, propagating from
Pupil to Focal planes. The focal_shift parameter represents the distance
from the focal plane at which the PSF is modelled. The shift parameter is used
to shift the center of the output plane by 'shift', which is treated in units
of pixels by default, otherwise it is treated in the units of the pixel_scale.
API
"""
