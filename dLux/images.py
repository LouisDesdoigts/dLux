...
The image attribute is a 2D array of the image. The pixel_scale is the
pixel scale of the image, which as units depending on the type of Optics used.

It has a number of methods that allow for the manipulation of the image. It
implements as series of operators that allow for the manipulation of the image
via standard arithmetic operators. ie:
...
## Downsample
Downsamples the image by a factor of n. This is done by summing the image
pixels in n x n blocks.
...
class Image(Base):
    """
    A class represnting some image as it is transformed by the detector. It
