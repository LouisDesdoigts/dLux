...
===================================
This module contains the classes that define the behaviour of ApertureLayers
in dLux.
...
Plus also one public method, ApertureFactory, that allows for the easy
construction of various simple apertures.

These classes are broken into three categories: Dyanmic Apertures, Aberrated
Apertures, and Composite Apertures.

The Dynamic apertures are apertures that are defined by physical units, such as
radius, and are generated at run-time on the input coordinates. These apertures
all have a set of input transformation parameters that can be used to modify the
shape of the aperture. These parameters are:
...
The Aberrated apertures are apertures that are defined by a set of Zernike
coefficients. These apertures and zernikes are generated at run-time on the
input coordinates. These classes contain a dynamic aperture, plus a dynamic
zernike basis and coefficients.

The Composite apertures are apertures that are defined by a set of sub-apertures
that are combined together. These apertures are generated at run-time on the
input coordinates. These classes contain a list of dynamic apertures that are
combined together. `CompoundApertures` are apertures that are combined together
via a multiplication (ie, combining a primary mirror and secondary mirror with
spiders), whereas `MultiApertures` are apertures that are combined together via
an addition (ie, combining different holes of an aperture mask).

Most users will not need to use these dynamic features, so all of these classes
have a `.make_static(npixels, diameter)` method that returns a static version of
the aperture that can be much faster to use. These static versions are
implemented as `Optic` classes, and can be used in the same way as any other
`Optic` class.

The primary interface for these classes is the `.transmission(npixels,
diameter)` method that returns the transmission of the aperture on a set of
coordinates defined by the number of pixels and diameter.
...
__all__ = [ "CircularAperture", "RectangularAperture", "RegPolyAperture",
    "IrregPolyAperture", "AberratedAperture", "UniformSpider",
...
class BaseDynamicAperture(ApertureLayer):
    ...
    def _coordinates(self : ApertureLayer, coordinates : Array) -> Array:
        ...
        """
        # Define and Apply Transformation Fucntions
...
class AberratedAperture(ApertureLayer, BasisLayer()):
    ...
    def __init__(self         : ApertureLayer, 
                 aperture     : ApertureLayer, 
                 noll_inds    : Array,
                 coefficients : Array = None) -> ApertureLayer: 
        """
        Parameters
        ----------
        aperture: ApertureLayer
    ...
    def __init__(self        : ApertureLayer,
        ...
        for aperture in self.apertures.values():
            if isinstance(aperture, AberratedAperture):
                naberrated += 1
            if naberrated > 1:
                raise TypeError("CompoundAperture can only have a single "
                    "AberratedAperture.")
