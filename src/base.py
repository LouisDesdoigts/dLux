...
class DevOpticalSystem(equinox.Module):
    """
    Base class defining some optical system
...
class Scene(equinox.Module):
    ...
    """
    optical_system: equinox.Module # Not static becuase we want to take grads through this?
    detector_layers: list
    wavels: jax.numpy.ndarray = equinox.static_field()
    positions: jax.numpy.ndarray
    fluxes: jax.numpy.ndarray
    weights: jax.numpy.ndarray
    dithers: jax.numpy.ndarray
    
    # Determined from inputs
    Nstars: int = equinox.static_field()
    Nwavels: int = equinox.static_field()
    Nims: int = equinox.static_field()
