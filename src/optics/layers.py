...
class Aperture(eqx.Module):
    ...
    def get_number_of_pixels(self : Layer) -> int:
        """
        Returns
        -------
        pixels : int
    ...
    def get_pixel_positions(self : Layer) -> Array:
        """
        """
    ...
    def __call__(self : Layer, parameters : Dict) -> Dict:
        ...
        wavefront = wavefront.mulitply_amplitude(
            self._aperture(self._number_of_pixels))
...
class AnnularAperture(Aperture):
    ...
    def _aperture(self : Layer) -> Array:
        ...
        aperture : Array[Float]
            ...
            coefficient of that pixel. 
        """
        centre = (self.get_number_of_pixels() - 1.) / 2.
        normalised_coordinates = (np.arange(npix) - centre) / centre
        stacked_grids = np.array(np.meshgrid(normalised_coordinates,
            normalised_coordinates)) 
        radial_coordinates = np.sqrt(np.sum(stacked_grids ** 2, axis = 0))
