...
@pytest.mark.parametrize(
    "layer",
    [
        dl.RefractiveOptic(1e-7, 1.5),
        dl.RefractiveOptic(1e-7, dl.CauchyIndex([1.5, 0.01])),
        dl.Wedge([1e-9, -2e-9], 1.5),
        dl.Wedge([1e-9, 0.0], dl.InterpolatedIndex([0.5e-6, 2e-6], [1.6, 1.5])),
    ],
)
...
def test_lens_is_chromatic(chromatic_wavefront):
    ...
    output = dl.RefractiveOptic(1e-7, index)(chromatic_wavefront)
